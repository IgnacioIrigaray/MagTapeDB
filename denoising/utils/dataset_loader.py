...
def noise_sample_generator(info_file,fs, length_seq, split, seed=43):
    ...
    while True:
        ...
        for i in r:
            ...
            if np.all(loaded_data == 0):
                print(f"🔇 Audio silencioso (todo ceros): ")
                breakpoint()
...
class ValDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, path_music, path_noises, fs=44100, seg_len_s=5):
        ...
        scales=np.random.uniform(-6,4,len(self.segments_clean))
        self.segments_clean=np.array(self.segments_clean)

        self.segments_clean=[10.0**(scales[i]/10.0) *self.segments_clean[i] for i in range(len(self.segments_clean))]
            ...
            for file in self.train_samples:  
                ...
                while(data_not_finished):
                    ...
                    if self.path_noises !=None:
                        ...
                        if np.isnan(data_noise).any():
                            print(f"❌ NaN detectado en noise")
                        if np.all(data_noise == 0):
                            print(f"🔇 Audio silencioso (todo ceros): noise")
                        #data_noise=np.mean(data_noise,axis=1)
                        #normalize
                        max_val = np.max(np.abs(data_noise))
                        if max_val > 0:
                            data_noise = data_noise / max_val
                        else:
                            print("⚠️ data_noise es todo ceros. Se saltea la normalización.")
                        data_noise=data_noise/np.max(np.abs(data_noise))
                        new_noise=data_noise #if more processing needed, add here
                        #load clean data
                        #configure sizes
